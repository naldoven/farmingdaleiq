...
import { completeWorkOrder, updateWorkOrderStatus } from "@/app/(app)/maintenance/actions";
 ...
 *
 * Dropping an active card on Complete completes it directly with blank
 * cost/invoice fields; the detail page still has the fuller completion form
 * when a cost or invoice photo needs to be recorded. Terminal statuses
 * (complete, cancelled) aren't draggable at all -- there's no valid transition
 * out of either. Cancelled work orders don't get a column (leader preference,
 * 2026-08-05): a cancelled order simply drops off the board; its page stays
 * reachable from equipment history.
 */
...
export function WorkOrderBoard({ workOrders }: { workOrders: WorkOrderRow[] }) {
  ...
  const drag = useKanbanDrag({
    ...
    onDrop: (cardId, column) => {
      ...
      if (column === "complete") {
        if (!isValidWorkOrderTransition(current.status as WorkOrderStatus, "complete")) {
          setError(`Can't move a ${columnLabel(current.status)} order to Complete.`);
          return;
        }

        setError(null);
        const previousStatus = current.status;
        setOrders((prev) => prev.map((wo) => (wo.id === cardId ? { ...wo, status: "complete" } : wo)));

        completeWorkOrder({ workOrderId: cardId }).then((result) => {
          if (!result.ok) {
                  ...
                  cards.map((wo) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <WorkOrderPhotoPreview title={wo.title} photos={wo.photo_urls} />
